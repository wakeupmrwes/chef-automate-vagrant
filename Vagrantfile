...
Vagrant.configure('2') do |config|
  ...
  config.vm.provision 'shell' do |s|
    s.inline = <<-SHELL
     ...
     echo "192.168.100.50 infranode" | tee -a /etc/hosts
    SHELL
  end
  ...
  config.vm.define 'Chef Server' do |chef_server|
    ...
    chef_server.vm.provision 'file', source: 'chef-server-core-12.8.0-1.el6.x86_64.rpm', destination: '/tmp/chef-server-core-12.8.0-1.el6.x86_64.rpm'
    chef_server.vm.provision 'file', source: 'chef-manage-2.4.0-1.el6.x86_64.rpm', destination: '/tmp/chef-manage-2.4.0-1.el6.x86_64.rpm'
    ...
    chef_server.vm.provision 'shell', inline: 'rpm -Uvh /tmp/chef-server-core-12.8.0-1.el6.x86_64.rpm'
    ...
    chef_server.vm.provision 'shell', inline: 'sudo chef-server-ctl install chef-manage --path /tmp/chef-manage-2.4.0-1.el6.x86_64.rpm'
    chef_server.vm.provision 'shell', inline: 'sudo chef-server-ctl reconfigure'
  ...
  config.vm.define 'Infra Node' do |infra_node|
    infra_node.vm.hostname = 'infranode'
    infra_node.vm.network 'private_network', ip: '192.168.100.50'
    infra_node.vm.provider :virtualbox do |vb|
      vb.memory = 512
      vb.cpus = 1
    end
  end
  ...
  config.vm.define 'Elasticsearch' do |elasticsearch|
    ...
    elasticsearch.vm.provision 'file', source: 'jre-8u101-linux-x64.rpm', destination: '~/jre-8u101-linux-x64.rpm'
    elasticsearch.vm.provision 'shell', inline: 'rpm -Uvh -i /home/vagrant/jre-8u101-linux-x64.rpm'
  ...
  config.vm.define 'Delivery Server' do |delivery_server|
    ...
    delivery_server.vm.provision 'file', source: 'delivery-0.5.1-1.el6.x86_64.rpm', destination: '~/delivery-0.5.1-1.el6.x86_64.rpm'
    delivery_server.vm.provision 'file', source: 'delivery.license', destination: '~/delivery.license'
    delivery_server.vm.provision 'shell', inline: 'rpm -Uvh -i /home/vagrant/delivery-0.5.1-1.el6.x86_64.rpm'
    ...
    delivery_server.vm.provision 'shell', inline: 'sudo delivery-ctl install-build-node -I /vagrant/chefdk-0.16.28-1.el6.x86_64.rpm -u vagrant -P vagrant -f buildnode'
